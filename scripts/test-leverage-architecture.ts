/**
 * TypeScript Testing script for the refactored leverage architecture
 * Tests: Mint shares → Place buy/sell orders → Execute match
 * 
 * Usage:
 *   ts-node scripts/test-leverage-architecture.ts [keypair-path] [market-address]
 *   OR with tsx: tsx scripts/test-leverage-architecture.ts [keypair-path] [market-address]
 * 
 * Example:
 *   ts-node scripts/test-leverage-architecture.ts ~/.config/solana/id.json <market-pda>
 */
...
// PDA helpers
function findProgramAddress(seeds: Buffer[], programId: PublicKey): PDAResult {
  return PublicKey.findProgramAddressSync(seeds, programId);
}

function getMarketPDA(creator: PublicKey, marketId: number): PDAResult {
...
function getMarketVaultPDA(market: PublicKey): PDAResult {
  return findProgramAddress(
...
function getMarginVaultPDA(market: PublicKey): PDAResult {
  return findProgramAddress(
...
function getLiquidityVaultPDA(market: PublicKey): PDAResult {
  return findProgramAddress(
...
function getVaultAuthorityPDA(market: PublicKey): PDAResult {
  return findProgramAddress(
...
function getMarginVaultAuthorityPDA(market: PublicKey): PDAResult {
  return findProgramAddress(
...
function getLiquidityVaultAuthorityPDA(market: PublicKey): PDAResult {
  return findProgramAddress(
...
function getYesMintPDA(market: PublicKey, outcomeId: number): PDAResult {
  return findProgramAddress(
...
function getNoMintPDA(market: PublicKey): PDAResult {
  return findProgramAddress(
...
function getMintAuthorityPDA(market: PublicKey): PDAResult {
  return findProgramAddress(
...
function getPendingOrderPDA(user: PublicKey, orderId: number): PDAResult {
...
function getOrderEscrowPDA(user: PublicKey, orderId: number): PDAResult {
  const orderIdBuffer = Buffer.allocUnsafe(8);
  orderIdBuffer.writeBigUInt64LE(BigInt(orderId), 0);
  return findProgramAddress(
...
function getOrderEscrowAuthorityPDA(user: PublicKey, orderId: number): PDAResult {
  const orderIdBuffer = Buffer.allocUnsafe(8);
  orderIdBuffer.writeBigUInt64LE(BigInt(orderId), 0);
  return findProgramAddress(
...
function getShareEscrowAuthorityPDA(user: PublicKey, orderId: number): PDAResult {
  const orderIdBuffer = Buffer.allocUnsafe(8);
  orderIdBuffer.writeBigUInt64LE(BigInt(orderId), 0);
  return findProgramAddress(
...
function getShareEscrowYesPDA(user: PublicKey, orderId: number): PDAResult {
  const orderIdBuffer = Buffer.allocUnsafe(8);
  orderIdBuffer.writeBigUInt64LE(BigInt(orderId), 0);
  return findProgramAddress(
...
function getPositionPDA(user: PublicKey, market: PublicKey, outcomeId: number, side: number): PDAResult {
  return findProgramAddress(
...
function getMatchStatePDA(market: PublicKey, buyOrderId: number, sellOrderId: number): PDAResult {
...
function getConfigPDA(): PDAResult {
...
async function main(): Promise<void> {
  console.log('🧪 Testing Refactored Leverage Architecture (TypeScript)');
  console.log('========================================================\n');
  ...
  if (!marketAddress) {
    ...
    console.error('   Usage: ts-node scripts/test-leverage-architecture.ts [keypair] <market-address>');
    process.exit(1);
  }
  ...
  const program = new Program(idl, SPACE_CORE_PROGRAM_ID, provider) as Program<Idl>;
  ...
  try {
    const [vaultAuthority] = getVaultAuthorityPDA(marketPDA);
